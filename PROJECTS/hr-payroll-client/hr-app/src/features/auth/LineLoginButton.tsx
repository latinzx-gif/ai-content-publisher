...
export function LineLoginButton({ lineStartUrl }: LineLoginButtonProps) {
  const { tx } = useLocale()

  return (
    <a
      href={lineStartUrl}
      className="flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-[#06C755] text-sm font-medium text-white transition-colors hover:bg-[#05b34c]"
    >
      <LineIcon />
      {tx("auth.login.lineButton")}
    </a>
  )
}
