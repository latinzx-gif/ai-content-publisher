...
import { StatusBadge } from "@/components/publisher/ai-law-content/StatusBadge";
import { PlatformIcon } from "@/components/publisher/ai-law-content/PlatformIcon";
import type { DemoPost, DemoPostStatus, DemoComment } from "@/lib/publisher/ai-law-content/types";
...
export default function ApprovalsPage() {
  ...
  function approve(id: string) {
    ...
    showToast(`${post?.title ?? "Post"} approved`);
  }

  function requestChanges(id: string) {
    updatePost(id, { status: "changes_requested" });
    const post = posts.find((p) => p.id === id);
    showToast(`${post?.title ?? "Post"} marked for changes`);
  }

  function reject(id: string) {
    rejectPost(id);
    const post = posts.find((p) => p.id === id);
    showToast(`${post?.title ?? "Post"} rejected`);
  }
  function sendComment(id: string, text: string) {
    const post = posts.find((p) => p.id === id);
    if (!post) return;
    ...
    updatePost(id, { comments: [...post.comments, nextComment] });
    showToast("Comment added to this post");
  }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-white">
      ...
      {toast && (
        <div
          className="fixed left-1/2 top-5 -translate-x-1/2 z-50 rounded-xl border border-[#bbf7d0] bg-[#ecfdf5] px-4 py-2 text-[12px] text-[#166534] shadow-lg"
          role="status"
          aria-live="polite"
        >
          {toast}
